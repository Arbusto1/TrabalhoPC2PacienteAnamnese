...
public class AntSistema {

    Paciente[] paciente = new Paciente[10];
    Anamnese[] anamnese = new Anamnese[10];
    Usuario[] usuarios = new Usuario[10];

    private static AntSistema instance;

    public static AntSistema getInstance() {
        if (instance == null) {
            instance = new AntSistema();
            return instance;
        }
        return null;
    }
            ...
            for (int i = 0; i < usuarios.length; i++) {
                ...
                if (usuarios[i] != null) {
                    ...
                    for (int j = 0; j < nome.length - 1; j++) {
                        if (usuarios[i].getNome() != null && usuarios[i].getLogin() != null && usuarios[i].getSenha() != null) {

                            if (j < usuarios[i].getNome().length()) {
                                nome[j] = usuarios[i].getNome().charAt(j);
                            } else {
                                nome[j] = ' ';
                            }

                            if (j < usuarios[i].getLogin().length()) {
                                login[j] = usuarios[i].getLogin().charAt(j);
                            } else {
                                login[j] = ' ';
                            }
    ...
    boolean verificaExiUsu(Usuario u) {
        if (u != null) {
            ...
            for (int i = 0; i < paciente.length; i++) {
                ...
                if (u.getNome() != null) {
                    if (usuarios[i] != null) {
                        if (u.getNome().equals(usuarios[i].getNome()) && u.getLogin().equals(usuarios[i].getLogin())) {
    ...
    boolean alterarUsuario(String nomeLogin, String senha, String Login, Usuario u) {
        ...
        for (int i = 0; i < usuarios.length; i++) {
            ...
            if (usuarios[i] != null && nomeLogin.equals(usuarios[i].getLogin())) {
                usuarios[i] = u;
                return true;
            }
        }
    ...
    boolean auxAlterarUsuario(String nomeLogin, String senha, String Login) {
        ...
        for (int i = 0; i < usuarios.length; i++) {
            ...
            if (usuarios[i] != null && nomeLogin.equals(usuarios[i].getLogin())
                    && Login.equals(usuarios[i].getNome()) && senha.equals(usuarios[i].getSenha())) {
                return true;
            }
        }
        return false;
    ...
    boolean excluirUsuario(String nomeLogin, String Login, String senha) {
        ...
        for (int i = 0; i < usuarios.length; i++) {
            ...
            if (usuarios[i] != null) {
                if (nomeLogin.equals(usuarios[i].getLogin()) && senha.equals(usuarios[i].getSenha())) {
                    usuarios[i] = null;
                    return true;
                }
            }
